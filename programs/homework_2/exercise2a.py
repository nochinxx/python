...
def main():
    ...
    output_file = os.path.join(
        output_directory,
        "answers",
        f"{get_exercise_name(exercise_file)}_frequency_table2.txt",
    )
    ...
    # Convert class_limits set to a sorted list
    class_limits_list = sorted(list(class_limits))

    # Plotting the frequency histogram
    plt.figure(figsize=(10, 6))

    # Plot the histogram bars
    plt.bar(class_limits_list, frequencies, width=1, align="center", alpha=0.7, edgecolor="black")

    # Set x-axis limits to start from 0
    plt.xlim(-0.5, len(class_limits_list) + 0.5)

    plt.xlabel("Number of Siblings")
    plt.ylabel("Frequency")
    plt.title(header[0])

    # Set x-axis ticks to the lower limit of each class
    plt.xticks(class_limits_list, class_limits_list)
